...
use libc::{self, c_int};
#[cfg(not(any(target_os = "solaris", target_os = "emscripten", target_os = "vxworks")))]
...
impl Socket {
    ...
    // ioctl(FIOCLEX) is not supported by VxWorks, Solaris/Illumos or emscripten,
    // use fcntl(FD_CLOEXEC) instead
    #[cfg(any(target_os = "solaris", target_os = "emscripten", target_os = "vxworks"))]
    pub fn new(family: c_int, ty: c_int) -> io::Result<Socket> {
        unsafe {
            let fd = try!(::cvt(libc::socket(family, ty, 0)));
